...
import diffuse_boost.spheres_in_cube
import diffuse_boost.spheres_in_cube.animation
...
class SphereDatasetEvaluator:
    ...
    def intersection_volume_two_spheres(self, center1,  center2, eps = 10**-6):
        ...
        dist = np.sum(diffuse_boost.spheres_in_Rd.physics_push.shortest_vector_torus(center1, center2, self.box_size)**2)**0.5
    ...
    def get_spheres_dist_sq(self, center1, center2):
        vec_ij = diffuse_boost.spheres_in_Rd.physics_push.shortest_vector_torus(center1, center2, self.box_size)
    ...
    def monte_carlo_union_volume(self, centers, num_samples=10_000):
        ...
        for c in centers:
            dist_sq = np.sum(diffuse_boost.spheres_in_Rd.physics_push.shortest_vector_torus(samples, c, self.box_size)**2, axis=1)
...
def plot_evaluations(data_evaluations, lowerbound, savepath, n_spheres, dimension, box_size, dt, tol, evaluation_skip, show=True):
    ...
    diffuse_boost.spheres_in_cube.animation.animate_histogram(Y5, savepath[:-4]+".gif", title=title, fps=max(1,len(data_evaluations)//2))
