...
from diffuse_boost.spheres_in_Rd.Max_Indep_Sets import MIS_basic, MIS_luby
import torch
...
def plot_and_sample_test():
    points = sample_poisson_point_process(dimension = cfg.getint("ppp_sample_generation_Rd", "dimension"), 
        bounding_box_width = cfg.getfloat("ppp_sample_generation_Rd", "bounding_box_width"), 
        intensity = cfg.getfloat("ppp_sample_generation_Rd", "intensity"))
    plt.scatter(points[0], points[1], c="black")
    thinned_points = MIS_luby(points.T, min_distance = cfg.getfloat("ppp_sample_generation_Rd", "sphere_radius"))
...
def generate_dataset(num_samples, min_size, filename):
    ...
    num_big_samples = 0
    dimension = cfg.getint("ppp_sample_generation_Rd", "dimension")
    ...
    while num_big_samples < num_samples:
        ...
        thinned_points = MIS_luby(points.T, min_distance=2*cfg.getfloat("ppp_sample_generation_Rd", "sphere_radius"))
