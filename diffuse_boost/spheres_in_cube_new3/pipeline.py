...
def main(state: "PipelineState | None" = None):
    """
    Run the standard pipeline loop of (optional) dataset generation, FM training/fine-tuning,
    sampling, and final push. The behaviour mirrors the script execution path so it can be
    invoked programmatically (e.g., from a radius search wrapper).
    """
    # Avoid circular imports
    from diffuse_boost.spheres_in_cube_new3.data_generation import _get_cfg, _set_cfg
    from diffuse_boost.spheres_in_cube_new3 import data_generation
    from diffuse_boost.spheres_in_cube_new3 import flow_matching_spheres
