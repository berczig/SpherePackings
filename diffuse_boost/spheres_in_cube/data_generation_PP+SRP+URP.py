...
def generate_dataset_push_srp():
    ...
    num_URP = cfg.getint(sec, "num_URP")
    sigma_URP_frac = cfg.getfloat(sec, "sigma_URP_frac")
    ...
    for i in range(M):
        ...
        for k in range(restarts):
            ...
            # URP+local_opt num_URP = 100 times
            excess_best = excess
            centers_best = centers_opt
            for _ in range(num_URP):
                X_urp = URP(X_lo, sigma_URP)
                X_urp_clip = np.clip(X_urp, -L/2 + eps, L/2 - eps)
                X_urp_lo, EL_after = local_opt(X_urp_clip, L, N, r, tol_opt, maxiter_opt)
                coords = X_urp_lo.reshape((N, 3))
                centers_opt = (coords + half) * ((L - 2 * r) / L) + r
                centers_opt = np.minimum(np.maximum(centers_opt, r + EPS_SMALL), L - r - EPS_SMALL)
                # Check the distance between spheres 
                diffs_k = centers_opt[:, None, :] - centers_opt[None, :, :]
                post_min = np.min(np.linalg.norm(diffs_k, axis=-1)[np.triu_indices(N, k=1)])
                excess = best_d - post_min
                print(f"    min distance after URP+local_opt = {post_min:.6f}, excess = {excess:.6f}")
                # Keep the best point set as centers_best 
                if excess < excess_best:
                    centers_best = centers_opt

            # Apply SRP+local_opt to centers_best
            # Apply SRP+local_opt to centers_best
            print("Applying SRP+local_opt to centers_best...")
            X_best = (centers_best - r) / ((L - 2 * r) / L) - half  # Map centers_best back to relative coordinates
            X_srp = SRP(X_best, L, N, r, Imax, m, sigma, beta)  # Apply SRP
            X_srp_clip = np.clip(X_srp, -L/2 + eps, L/2 - eps)  # Clip to valid bounds
            EL_before = compute_EL(X_srp_clip, L, N, r)
            print(f"    SRP on centers_best: EL before local_opt = {EL_before:.6f}")

            # Apply local optimization
            X_lo, EL_after = local_opt(X_srp_clip, L, N, r, tol_opt, maxiter_opt)
            print(f"    SRP on centers_best: EL after local_opt  = {EL_after:.6f}")
            ...
            post_min = np.min(np.linalg.norm(diffs_k, axis=-1)[np.triu_indices(N, k=1)])
            excess = best_d - post_min
            print(f"    min distance after SRP+local_opt = {post_min:.6f}, excess = {excess:.6f}")

            # Update centers_best if the new configuration is better
            if post_min > best_min:
                best_min = post_min
                centers_best = centers_opt.copy()
                print(f"    Updated centers_best with new min distance = {post_min:.6f}")
            ...
            post_min = np.min(np.linalg.norm(diffs_k, axis=-1)[np.triu_indices(N, k=1)])
            excess = best_d - post_min
            print(f"    min after physics push = {post_min:.6f}, excess = {excess:.6f}")
