...
class FlowSetTransformer(nn.Module):
    def __init__(self, d=3, **st_kwargs):
        super().__init__()
        self.time_emb = TimeEmbedding(d)
        if "dim_in" not in st_kwargs:
            st_kwargs["dim_in"] = 2*d
...
def train_flow_model(
    ...
    for epoch in tqdm(range(num_epochs), desc="Training"):
        ...
        for x0 in loader:
            ...
            x0_proj= x0_pred.clamp(sphere_radius, clip_max - sphere_radius) #center + half*torch.tanh((x0_pred-center)/half)
...
def sample_flow_model(model, num_samples, batch_size, num_points,
    ...
    for _ in tqdm(range(int(np.ceil(num_samples/batch_size))), desc="Inference"):
        ...
        with torch.no_grad():
            ...
            proj= out.clamp(sphere_radius, clip_max - sphere_radius) #center + half*torch.tanh((out-center)/half)
...
if __name__ == '__main__':
    ...
    #lambda_lr = lambda epoch: np.exp(np.interp(epoch, [0, int(epochs*0.1), epochs],[np.log(lr), np.log(lr*0.4), np.log(eta_min)]))
    lambda_lr = lambda epoch: np.exp(np.interp(epoch, [0, epochs],[np.log(lr), np.log(eta_min)]))
