...
if __name__ == "__main__":
    ...
    for sim in range(simulations):
        ...
        plot_evaluations(
            ...
            savepath=os.path.join(SP.reffolder, f"output/fixed_gen_sets/{s_now}/eval{sim+1:05d}.png")
        )
    ...
    savepath = os.path.join(SP.reffolder, f"output/fixed_gen_sets/{s_now}/dataset.pt")
    ...
    plot_evaluations(
        ...
        box_size=box_size, dt=dt, tol=tol, evaluation_skip=evaluation_skip, show=False,
        savepath=os.path.join(SP.reffolder, f"output/fixed_gen_sets/{s_now}/best_min_dist.png")
    )
    plot_evaluations(
        best_ratio, lower_bound, n_spheres=n_points, dimension=dimension,
        box_size=box_size, dt=dt, tol=tol, evaluation_skip=evaluation_skip, show=False,
        savepath=os.path.join(SP.reffolder, f"output/fixed_gen_sets/{s_now}/best_ratio.png")
    )