...
from SP._100.unet import UNet

# Taken from: https://github.com/MaximeVandegar/Papers-in-100-Lines-of-Code/tree/main/Denoising_Diffusion_Probabilistic_Models
...
def main():
    ...
    for i in range(nb_images):
        plt.subplot(9, 9, 1 + i)
        plt.axis('off')
        print(type(samples[i]))
        print(samples[i])
        plt.imshow(samples[i].squeeze(0).clip(0, 1).data.cpu().numpy(),
                   cmap='gray')
    plt.savefig(os.path.join(SP.reffolder, "output/samples.png"))